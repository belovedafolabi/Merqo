...
- Idempotency is verified in CI by `supabase start` (which applies migrations + seed once) followed by a single `supabase db reset` (see `.github/workflows/ci.yml`, job `db-migrations`) — since `db reset` always drops and rebuilds the schema from scratch, a from-scratch apply failure is caught immediately; the second seed application is what exercises `seed.sql`'s `on conflict ... do nothing` idempotency. Milestone 16 dropped a third redundant `db reset` that added ~1–2 min and proved nothing beyond the two applications.
